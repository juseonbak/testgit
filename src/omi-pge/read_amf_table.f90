SUBROUTINE read_amf_table ( pge_idx )
  ...
  ! ==============================
  CHARACTER (LEN=14), PARAMETER :: modulename = 'read_amf_table'
  ...
  ! -----------------------
  omi_lun = pge_static_input_luns(amf_idx,pge_idx)
  version = 1
  errstat = PGS_IO_GEN_OPENF ( omi_lun, PGSd_IO_Gen_RSeqFrm, 0, funit, version )
  IF ( PGS_SMF_TESTSTATUSLEVEL(errstat) /= pgs_smf_mask_lev_s .AND. &
       verb_thresh_lev >= vb_lev_omidebug ) THEN
     errstat = OMI_SMF_setmsg ( &
          omsao_w_open_amftable_file, TRIM(ADJUSTL(static_input_fnames(amf_idx))), modulename, 0)
