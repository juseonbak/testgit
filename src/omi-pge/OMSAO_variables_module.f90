...
MODULE OMSAO_variables_module
  ...
  ! -------------------------------------
  INTEGER, PARAMETER :: omi_idx = 1, gome_idx = 2, scia_idx = 3, gome2_idx =4, omps_idx=5
  INTEGER, PARAMETER :: max_instrument_idx = gome2_idx
  CHARACTER (LEN=4), DIMENSION (max_instrument_idx), PARAMETER :: &
       which_instrument = (/ 'OMI ', 'GOME', 'SCIA', 'GOM2'/)
  INTEGER :: instrument_idx
  INTEGER, PARAMETER :: slit_ins_idx=5
  ...
  LOGICAL :: use_calc_sig ! define Sa from RMSE of fitting residuals, now just for OMPS
  ! -------------------------------------------
  ...
  REAL (KIND=dp),    DIMENSION (max_fit_pts)    :: fitspec_rad, fitres_rad, actspec_rad, simspec_rad, clmspec_rad, cmcspec
  ...
  INTEGER                           :: the_month, the_year, the_day , thedoy
  INTEGER                           :: nview, nloc
  ...
  ! calibration
  ! ----------------------
  INTEGER                                :: numwin, nxbin, nybin
  LOGICAL                                :: do_ybin, do_xbin
  ...
  ! JBAK 
  INTEGER :: here_stop, here_print, curr_x, curr_y, nx_pix, ny_line
  LOGICAL :: write_is_slit
  CHARACTER(LEN=maxchlen) :: raddate
  REAL(KIND=DP), DIMENSION(max_fit_pts, 3, max_fit_pts) :: calfitspec
