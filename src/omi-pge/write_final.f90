...
SUBROUTINE omi_write_intermed (founit, fitcol, dfitcol, rms, avgres,exval )
  ...
  USE OMSAO_omidata_module,   ONLY : land_water_flg, glint_flg, snow_ice_flg, offset_line, nxbin, nybin
  ...
  REAL (KIND=dp), INTENT (OUT) :: avgres
  ...
  IF (exval >= 0) THEN
     ...
     WRITE(founit, '(A)') 'Measurement Reflectance at 347nm: '
     WRITE(founit, '(3F9.4)') measref  , simref  , (measref-simref)*100/simref
...
SUBROUTINE gome_write_intermed (founit, npix, nsub, fitcol, dfitcol, rms, amf, amfgeo, &
  ...
  USE ozprof_data_module,      ONLY: ozprof_flag, ozprof, ozprof_std, ozprof_init, &
       ozprof_ap, ozprof_apstd, eff_alb, eff_alb_init, nlay, ozdfs, ozinfo, num_iter, &
       covar, contri, avg_kernel,favg_kernel, use_lograd, use_oe, nalb, atmosprof, ntp, nlay_fit, &
       ozprof_start_index, ozprof_end_index, ozfit_start_index, ozfit_end_index, &
       ...
       saa_flag, nsaa_spike, ozwrtfavgk,  nsfc
...
SUBROUTINE omps_write_cali (what_is_write, founit, n_slitvar, slitidx, slitname,first_pix, last_pix, first_line, last_line)
  USE OMSAO_parameters_module, ONLY: max_fit_pts
  USE OMSAO_variables_module, ONLY : yn_varyslit, numwin, wavcal, wavcal_sol

  USE OMSAO_precision_module
  USE OMSAO_omidata_module , ONLY: nxtrack_max, omi_nwav_irrad,omi_cali_sol,& 
                                omi_solwinfit,omi_wincal_wav, omi_nslit_sol, omi_solslitfit, omi_slitwav_sol, &
                                !omps_radwinfit,omi_nslit_rad, omps_radslitfit,omps_slitwav_rad, &
                                omi_nwavcal_sol,omi_solwavfit,omi_sswav_sol
                               ! omps_radwavfit,omps_sswav_rad,omps_nwavcal_rad,&
  IMPLICIT NONE
  ! IN/OUT varialbes
  INTEGER, INTENT(IN) :: founit, n_slitvar, first_pix, last_pix, first_line,last_line , what_is_write
  INTEGER, DIMENSION ( n_slitvar), INTENT(IN) :: slitidx
  CHARACTER (len=100), DIMENSION (n_slitvar), INTENT(IN) :: slitname
  ! LOCAL
  INTEGER :: n_slitwav, currpix, currloop, i, ntmp
  REAL (KIND=dp),DIMENSION (max_fit_pts) :: slitwav
  REAL (KIND=dp),DIMENSION (max_fit_pts, n_slitvar) :: slitfit


  IF (what_is_write == 1) THEN
  n_slitwav = numwin

  DO currpix = first_pix, last_pix
     ntmp =  omi_nwav_irrad(currpix)
     slitwav (1:n_slitwav) = omi_wincal_wav(1:n_slitwav, currpix)
     slitfit (1:n_slitwav, 1:n_slitvar) = omi_solwinfit(1:n_slitwav,slitidx(1:n_slitvar),1, currpix)
     IF (yn_varyslit)  then
           n_slitwav = omi_nslit_sol(currpix)
           slitfit (1:n_slitwav, 1:n_slitvar) = omi_solslitfit(1:n_slitwav,slitidx(1:n_slitvar),1, currpix)
           slitwav (1:n_slitwav) = omi_slitwav_sol(1:n_slitwav, currpix)
           IF (wavcal_sol) THEN
           n_slitwav = omi_nwavcal_sol(currpix)
           slitfit (1:n_slitwav, 1:n_slitvar) = omi_solwavfit(1:n_slitwav,slitidx(1:n_slitvar),1, currpix)
           slitwav (1:n_slitwav) = omi_sswav_sol(1:n_slitwav, currpix)
           ENDIF
     ENDIF

     WRITE(founit, '(5I5)') currpix, 0, n_slitwav, ntmp
     WRITE(founit,'(a10,1p8d11.3)') 'avgwav', slitwav (1:n_slitwav)
     DO i = 1, n_slitvar
        WRITE(founit,'(a10,1p8d11.3)') slitname(i), slitfit(1:n_slitwav, i )
     ENDDO
     DO i = 1, n_slitwav
           WRITE(founit,'(200f8.3)')    omi_cali_sol(i,1,1:ntmp,currpix)
           WRITE(founit,'(1p200d11.3)') omi_cali_sol(i,2,1:ntmp,currpix)
           WRITE(founit,'(1p200d11.3)') omi_cali_sol(i,3,1:ntmp,currpix)
     ENDDO
  ENDDO

  ENDIF
  RETURN
END SUBROUTINE omps_write_cali
