SUBROUTINE subtract_cubic ( locwvl, npts, ll_rad, lu_rad )
  ...
  REAL (KIND=dp), DIMENSION (doas_npol)        :: blow, bupp
  REAL (KIND=dp), DIMENSION (max_spec_pts)           :: f
  REAL (KIND=dp), DIMENSION (max_spec_pts,doas_npol) :: dfda
  REAL (KIND=dp), DIMENSION (doas_npol)        :: par
  ...
  DO i = 1, max_rs_idx
     IF ( i /= solar_idx ) THEN
        ...
        blow(1:doas_npol) = 0.0_dp  ;  bupp(1:doas_npol) = 0.0_dp
...
SUBROUTINE subtract_cubic_meas ( locwvl, npoints, locspec, ll_rad, lu_rad )
  ...
  REAL (KIND=dp), DIMENSION (doas_npol)           :: par, polfunc, r
  REAL (KIND=dp), DIMENSION (doas_npol,doas_npol) :: covar
  ...
  REAL (KIND=dp), DIMENSION (npoints,doas_npol) :: dfda
  ...
  !     Find limits for polynomial fitting, with ~1 nm overlap
  nlower = MINVAL(MINLOC( locwvl(1:npoints), MASK=(locwvl(1:npoints) >= locwvl(ll_rad)-1.0_dp) ))
  nupper = MAXVAL(MAXLOC( locwvl(1:npoints), MASK=(locwvl(1:npoints) <= locwvl(lu_rad)+1.0_dp) ))
  nfitted = nupper - nlower + 1
