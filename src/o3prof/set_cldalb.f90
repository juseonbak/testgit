...
SUBROUTINE SETCLD(npoints, fitwavs, nscene, ctaus, ctps, cfracs, errstat)
  ...
  USE OMSAO_precision_module
  USE OMSAO_variables_module,    ONLY: the_lon, the_lat, the_month, the_year, &
       the_day, b1ab_div_wav
  USE ozprof_data_module,        ONLY: cloud, do_ch2reso
  USE OMSAO_gome_data_module,    ONLY: gome_pixdate, gome_orbc, gome_curpix, &
       gome_curscan, gome_npix
  USE OMSAO_errstat_module
  ...
  REAL(KIND=dp), DIMENSION(npoints)   :: fitwavs
  REAL (KIND=dp), DIMENSION (nscene), INTENT(OUT) :: ctaus, ctps, cfracs ! 0.375s and 1.2s
 
  ! ===============
  ! Local variables
  ! ===============
  INTEGER :: i, j, k, stpix, sndpix, cldflg

  errstat = pge_errstat_ok
  ctaus  = 0.0; cfracs = 0.0; ctps   = 0.0

  IF (cloud)  THEN
     IF (gome_npix == 1 .AND. gome_curscan < 3) THEN  
        stpix = gome_curpix;       cldflg = 1      
     ELSE IF (gome_npix == 1 .AND. gome_curscan == 3) THEN
        stpix = gome_curpix - 3;   cldflg = 2
     ELSE IF (gome_npix == 8) THEN
        stpix = gome_curpix - 7;   cldflg = 0
     ELSE IF (gome_npix == 40) THEN
        stpix = gome_curpix - 39;  cldflg = 5
     ELSE
        WRITE(*, *) 'SETCLD: Should not happen'
        errstat = pge_errstat_error; RETURN
     ENDIF
     
     ! always assume the same cloud info for channel 1 and channel 2
     ! current algorithm cannot handle different cloud information for chs 1 & 2
     sndpix = 0                 
       
     !WRITE(*,'(3I5,A4,3I5, 2F8.2)') the_year, the_month, the_day, &
