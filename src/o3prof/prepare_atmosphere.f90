...
SUBROUTINE get_tbprof (ozref, out_prof)
  ...
  USE OMSAO_precision_module 
  USE ozprof_data_module,     ONLY:  trpz, which_clima, which_aperr
...
SUBROUTINE get_ab (ozref,std)
  ...
  LOGICAL, SAVE                 :: first = .TRUE.
  REAL (KIND=dp), SAVE, DIMENSION(nmon, nlat, nlay) ::ozrefs
...
SUBROUTINE get_mcprof(ozref)
  ...
  CHARACTER (LEN=131)                                :: ozprof_fname
...
!    pressure from all years
! xliu: 03/08/11, switch from NCEP to 1x1 NCEP FNL (archived at NCAR)
! Jbak: 09/15/11, adding which_tpres to select NCEP or AIRS
 ! =================================================================
...
!!$       0.0491, 0.0348, 0.0245, 0.0148, 0.0068, 0.0029, 0.0011, 0.0004, 0.0001, 0.0000/)
  REAL (KIND=DP), DIMENSION(0:nalt_post2006), PARAMETER:: etae    = (/  &
    ...
    0.0573,  0.0408, 0.0288, 0.0201, 0.0094, 0.0041, 0.0017, 0.0006, 0.0002, 0.0001, 0.0000/)
...
SUBROUTINE get_mlso3prof_single(year, month, day, lat, nz, mnorstd, ps, zs, oz, ntp, errstat)
  ...
  OPEN (UNIT = atmos_unit, file = 'INP/mlsprof_index.inp', status = 'unknown', IOSTAT=ios)
  IF (ios /= 0) THEN
